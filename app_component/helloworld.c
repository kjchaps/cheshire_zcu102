...
int main(void) {
  ...
  int print_to_file = 0; //1 if print arrays to file, 0 is not
  //do not sure on very large array lengths- only for float
  
  setup(in_file);

  while (1) {
    test_counter = test_counter + 1;
    xil_printf("Running Test %d\n", test_counter);
    uint32_t finalRun = runTest(test_counter, print_to_file);
    if (finalRun)
      break;
  }
